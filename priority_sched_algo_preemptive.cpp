// Priority Scheduling Algorithm - ( Mode = Preemptive )
// Higher the number, higher the priority ( Considering condition )
...
struct Process{
	int PID,AT,BT,NBT,CT,TAT,WT,RT,CFT,PR; 
	// NBT - New Best Time
	// CFT - CPU First Time
	// PR - Priority number
};
...
int main(){
	...
	for(int i=0;i<no_of_process;i++){
		...
		scanf("%d",&bt);
		printf("Enter process Priority number: ");
		scanf("%d",&p[i].PR);
		p[i].BT = bt;
		p[i].NBT = bt;
			...
			if(p[i].NBT!=0){ // checking if process completed or not
				if(p[i].AT<=t){ // checking if arrival time of process is less than t (running time of program)
					if(currentProcess!=-1){ // if current process is not -1, then already some process is selected, then we need to check thier burst time.
						if(p[i].PR>p[currentProcess].PR){ // checking priority of process
							currentProcess = i;
							continue;
						}
						if(p[i].PR==p[currentProcess].PR){ // If process has same priority, check its Arrival Time
							if(p[i].AT<p[currentProcess].AT){
								currentProcess = i;
								continue;
							}
							if(p[i].AT==p[currentProcess].AT){ // if arrival time of process are smae, then consider their Process id;
								if(p[i].PID<p[currentProcess].PID){
									currentProcess = i;
								}
							}
						}
